...
public class ListedBrancheRepositoryPlugin extends AnalyzerGitLogPlugin {
    public static int nbcommits=0;

    public ListedBrancheRepositoryPlugin(Configuration generalConfiguration) {
    ...
    public void run() {
        if(!configuration.githubOrNormal()){
            if(listCommits==null)
                result = processLog(Parsing.parseLogFromCommand(configuration.getGitPath(),configuration.buildCommand("Branche")));
            else
    ...
    static class Result implements AnalyzerPlugin.Result {
        ...
        public String getResultAsHtmlDiv() {
            ...
            try {
                URL url = new URL("https://gaufre.informatique.univ-paris-diderot.fr/api/v4/projects/3327/repository/branches");
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    bf.write("<!DOCTYPE html>\n" +
                            ...
                            "  var Branch = \"<div class=\\'title\\'>\"+ aRepresentative.name +\"</div>\";\n" +
                            "  var Dateissue = \"<li>Created Date :</li> \"+ aRepresentative.commit.created_at + \"\";\n" +
                            "  var author = \"<li>Author :</li>\" + aRepresentative.commit.author_name + \"\";\n" +
                            "  var Status = \"<li>Merged :</li>\" + aRepresentative.merged + \"\";\n" +
                            "  divElementCollection.append(Branch);\n" +
                            "  divElementCollection.append(Dateissue);\n" +
                            "  divElementCollection.append(author);\n" +
                            "  divElementCollection.append(Status);\n" +
                            "}\n" +
                            "\n" +
